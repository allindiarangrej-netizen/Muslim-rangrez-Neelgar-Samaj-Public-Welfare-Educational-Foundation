...
print("Building 108 Verified Medical Colleges TS File...")
...
# Replace interface optional fields
if "city?: string;" not in text and "city: string;" not in text:
    text = text.replace("district: string;\n", "district: string;\n  city?: string;\n")
if "logoUrl?: string;" not in text:
    text = text.replace("name: string;\n", "name: string;\n  logoUrl?: string;\n  coverImageUrl?: string;\n")
if "offeredCourses?: MedicalCourse[];" not in text:
    text = text.replace("course: MedicalCourse;\n", "course: MedicalCourse;\n  offeredCourses?: MedicalCourse[];\n")
if "accreditation?: string;" not in text:
    text = text.replace("regulatoryAuthority: RegulatoryAuthority;\n", "regulatoryAuthority: RegulatoryAuthority;\n  accreditation?: string;\n  nirfRanking?: string;\n")
if "placementInformation?: string;" not in text:
    text = text.replace("scholarshipInfo: string;\n", "scholarshipInfo: string;\n  placementInformation?: string;\n  entranceExam?: string;\n")
if "admissionPortalUrl?: string;" not in text:
    text = text.replace("admissionProcess: string;\n", "admissionProcess: string;\n  admissionPortalUrl?: string;\n")
if "campusGallery?: string[];" not in text:
    text = text.replace("officialRegistrySearchUrl: string;\n", "officialRegistrySearchUrl: string;\n  campusGallery?: string[];\n  prospectusUrl?: string;\n  lastVerifiedDate?: string;\n")
...
with open(ts_file, "w", encoding="utf-8") as f:
    f.write(header_code + ts_array)

print(f"Successfully generated initial {len(colleges)} colleges array.")
