...
print("Building complete 110+ medical colleges data generator script...")

data_file_path = "/src/data/medicalCollegesData.ts"

# Let's prepare a python script that outputs the full file /src/data/medicalCollegesData.ts
py_code = '''import json

colleges = []

def c(id, name, course, offered, type_, minority, state, district, city, address, univ, reg_app, reg_auth, acc, nirf, year, intake, fee_cat, fee_val, fee_note, hostel_det, scholar, placement, exam, neet, adm_proc, adm_url, counsel_link, counsel_auth, maps, phone, email, web, reg_url):
    colleges.append({
        "id": id,
        "name": name,
        ...
        "hostelAvailability": {
            "available": True if "N/A" not in hostel_det and "No" not in hostel_det else False,
            "details": hostel_det
        },
