  ...
  return (
    <footer className="bg-[#061A0E] text-white py-16 relative overflow-hidden" id="corporate_footer">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12" id="footer_grid">
        ...
        <div className="space-y-4">
          <h4 className="text-xs font-bold text-[#F4C430] uppercase tracking-widest border-b border-white/10 pb-2">
            {currentLanguage === 'en' ? 'CONNECT WITH US' : currentLanguage === 'ur' ? 'ہم سے رابطہ کریں' : 'अधिकारिक सोशल मीडिया'}
          </h4>
          <p className="text-xs text-gray-400 leading-normal">
            {currentLanguage === 'en'
              ? 'Join our official YouTube, Facebook, WhatsApp, Telegram, Instagram & Twitter handles for live announcements and assemblies.'
              : currentLanguage === 'ur'
              ? 'سرکاری اعلانات کے لیے ہمارے یوٹیوب، فیس بک، واٹس ایپ، ٹیلی گرام اور انسٹاگرام سے جڑیں۔'
              : 'अधिकारिक घोषणाओं के लिए हमारे यूट्यूब, फेसबुक, व्हाट्सएप, टेलीग्राम और इंस्टाग्राम से जुड़ें।'}
          </p>

          <div className="flex flex-wrap items-center gap-3 pt-2" id="social_media_chips">
            <SocialButton platform="facebook" url={COMMUNITY_SOCIAL_URLS.facebook} title="Facebook Group" size="md" variant="solid" />
            <SocialButton platform="youtube" url={COMMUNITY_SOCIAL_URLS.youtube} title="YouTube Channel" size="md" variant="solid" />
            <SocialButton platform="whatsapp" url={COMMUNITY_SOCIAL_URLS.whatsappChannel} title="WhatsApp Channel" size="md" variant="solid" />
            <SocialButton platform="telegram" url={COMMUNITY_SOCIAL_URLS.telegram} title="Telegram Circle" size="md" variant="solid" />
            <SocialButton platform="instagram" url={COMMUNITY_SOCIAL_URLS.instagram} title="Instagram Page" size="md" variant="solid" />
            <SocialButton platform="twitter" url={COMMUNITY_SOCIAL_URLS.twitter} title="Twitter / X" size="md" variant="solid" />
            <SocialButton platform="linkedin" url={COMMUNITY_SOCIAL_URLS.linkedin} title="LinkedIn" size="md" variant="solid" />
          </div>
        </div>
