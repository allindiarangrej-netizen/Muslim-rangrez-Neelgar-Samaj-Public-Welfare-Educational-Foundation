...
import { SCIENCE_COLLEGES, ScienceCollegeProfile } from '../data/scienceCollegesData';

interface ScienceCollegesDirectoryProps {
  currentLanguage: 'en' | 'ur' | 'hi';
}

export default function ScienceCollegesDirectory({ currentLanguage = 'en' }: ScienceCollegesDirectoryProps) {
  const [activeTab, setActiveTab] = useState<'directory' | 'ug' | 'pg' | 'research' | 'specializations'>('directory');
  ...
  // Modal Detail State
  const [selectedCollege, setSelectedCollege] = useState<ScienceCollegeProfile | null>(null);
  ...
  const statesList = useMemo(() => {
    const states = new Set(SCIENCE_COLLEGES.map(c => c.state));
    return ['All', ...Array.from(states).sort()];
  }, []);

  const citiesList = useMemo(() => {
    const filtered = SCIENCE_COLLEGES.filter(c => selectedState === 'All' || c.state === selectedState);
  ...
  const filteredColleges = useMemo(() => {
    let result = [...SCIENCE_COLLEGES];

    // Tab filtering
    if (activeTab === 'ug') {
      result = result.filter(c => c.programmes.some(p => p.includes('B.Sc') || p.includes('BS')));
    } else if (activeTab === 'pg') {
      result = result.filter(c => c.programmes.some(p => p.includes('M.Sc') || p.includes('MS')));
  ...
  return (
    <div className="w-full bg-[#f8fafc] min-h-screen text-slate-800 font-sans pb-16">
      ...
      {/* HERO BANNER */}
      <div className="bg-gradient-to-br from-[#0f172a] via-[#1e293b] to-[#0f172a] text-white pt-10 pb-12 px-4 sm:px-8 border-b-4 border-emerald-500 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-emerald-900/40 border border-emerald-400/30 text-emerald-100 text-xs font-bold uppercase tracking-widest shadow-xl">
              <FlaskConical className="w-3.5 h-3.5 text-emerald-400" />
              <span>
                {currentLanguage === 'en'
                  ? 'National Science Education Directory'
                  : currentLanguage === 'ur'
                  ? 'قومی سائنس ایجوکیشن ڈائریکٹری'
                  : 'राष्ट्रीय विज्ञान शिक्षा निर्देशिका'}
              </span>
            </div>

            <div className="flex items-center gap-2 text-xs font-bold text-blue-400 bg-blue-950/50 border border-blue-500/30 px-3 py-1.5 rounded-xl shadow-lg">
              <ShieldCheck className="w-4 h-4" />
              <span>
                {currentLanguage === 'en'
                  ? '50+ Verified Science Institutions'
                  : '50+ تصدیق شدہ سائنسی ادارے'}
              </span>
            </div>
          </div>
          ...
          <h1 className="text-3xl sm:text-5xl font-black text-white tracking-tight leading-tight mb-4">
            {currentLanguage === 'en'
              ? 'Premier Science Colleges & Research Institutes'
              : currentLanguage === 'ur'
              ? 'اعلیٰ سائنسی کالجز اور تحقیقی ادارے'
              : 'प्रमुख विज्ञान कॉलेज और अनुसंधान संस्थान'}
          </h1>

          <p className="text-base text-slate-300 max-w-3xl leading-relaxed mb-8 font-medium">
            Explore India's leading Science institutions, from world-class research centers like IISc and IISERs to top-ranked university departments. Find B.Sc, M.Sc, and Ph.D programmes across Physics, Biology, Chemistry, and emerging scientific fields.
          </p>
          ...
          <div className="flex flex-wrap items-center gap-3">
            {[
              ...
              { id: 'ug', label: 'Undergraduate (B.Sc)', icon: BookOpen },
              { id: 'pg', label: 'Postgraduate (M.Sc)', icon: GraduationCap },
              { id: 'research', label: 'Research & Ph.D', icon: Microscope },
              { id: 'specializations', label: 'Subject Guide', icon: Atom }
            ].map(tab => (
              <button
                key={tab.id}
                ...
                className={`px-5 py-3 rounded-2xl text-xs sm:text-sm font-bold transition-all flex items-center gap-2 shadow-lg border-2 ${
                  activeTab === tab.id ? 'bg-emerald-600 text-white border-white' : 'bg-slate-800/60 text-slate-300 hover:bg-slate-700/80 border-slate-700/50'
                }`}
              >
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white p-6 sm:p-8 rounded-[2.5rem] shadow-xl border border-slate-200 space-y-6">
              <div className="flex flex-col lg:flex-row gap-4 items-center">
                <div className="relative flex-1 w-full">
                  ...
                  <input
                    type="text"
                    placeholder="Search by college name, city, or subject (Astrophysics, Biotech)..."
                    value={searchQuery}
                    onChange={(e) => { setSearchQuery(e.target.value); setCurrentPage(1); }}
                    className="w-full pl-12 pr-4 py-3.5 rounded-2xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm font-bold bg-slate-50 shadow-inner"
                  />
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4 pt-6 border-t border-slate-100">
                ...
                <div className="flex items-end">
                  <label className="flex items-center gap-3 cursor-pointer bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-2.5 w-full">
                    ...
                    <span className="text-xs font-black text-emerald-700">Placement Cell</span>
                  </label>
                </div>
              </div>
            </div>
                  ...
                  <div className="h-40 relative overflow-hidden">
                    ...
                    <div className="absolute bottom-4 left-4 right-4 flex justify-between items-end">
                      <div className="flex gap-2">
                        <span className="bg-emerald-600 text-white text-[9px] font-black uppercase px-2 py-0.5 rounded-lg border border-white/30">NIRF #{college.nirfRanking}</span>
                  ...
                  <div className="p-6 flex-1 flex flex-col">
                    <div className="flex items-start gap-4 mb-4">
                      ...
                      <div className="flex-1">
                        <h3 className="text-base font-black text-slate-900 group-hover:text-emerald-600 transition leading-tight mb-1">
                          {college.name}
                        </h3>
                    ...
                    <div className="mb-6">
                      <span className="text-[9px] font-black text-slate-400 uppercase block mb-2">Key Subjects</span>
                      <div className="flex flex-wrap gap-1.5">
                        {college.subjects.slice(0, 3).map((sub, idx) => (
                          <span key={idx} className="bg-emerald-50 text-emerald-700 text-[10px] font-black px-2.5 py-1 rounded-lg border border-emerald-100">
                            {sub}
                          </span>
                        ))}
                        {college.subjects.length > 3 && (
                          <span className="text-[10px] font-black text-slate-300">+{college.subjects.length - 3}</span>
                        )}
                      </div>
                    </div>
                    ...
                    <button
                      ...
                      className="w-full mt-auto bg-slate-900 text-white text-xs font-black py-4 rounded-2xl hover:bg-emerald-600 transition flex items-center justify-center gap-2 group/btn shadow-lg"
                    >
                      View Full Profile
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-3 pt-10">
                <button onClick={() => setCurrentPage(prev => Math.max(1, prev - 1))} disabled={currentPage === 1} className="w-12 h-12 flex items-center justify-center bg-white border border-slate-200 rounded-2xl text-slate-400 hover:text-emerald-600 disabled:opacity-40 shadow-sm transition"><ChevronRight className="w-6 h-6 rotate-180" /></button>
                {Array.from({ length: totalPages }, (_, i) => i + 1).map(num => (
                  <button key={num} onClick={() => setCurrentPage(num)} className={`w-12 h-12 rounded-2xl text-sm font-black transition-all ${currentPage === num ? 'bg-emerald-600 text-white shadow-xl scale-110' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 hover:border-emerald-400 shadow-sm'}`}>{num}</button>
                ))}
          ...
          <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-slate-200">
              <h3 className="text-2xl font-black text-slate-900 mb-8 flex items-center gap-3">
                <Atom className="w-8 h-8 text-emerald-600" />
                Science Disciplines & Specializations
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                <div className="space-y-4">
                  <div className="bg-emerald-50 p-6 rounded-3xl border border-emerald-100">
                    <h4 className="text-lg font-black text-emerald-900 mb-2">Physical Sciences</h4>
                    <p className="text-xs font-bold text-emerald-700 leading-relaxed">Physics, Applied Physics, Astrophysics, Astronomy, Nanotechnology, and Material Science.</p>
                  </div>
                  <div className="bg-blue-50 p-6 rounded-3xl border border-blue-100">
                    <h4 className="text-lg font-black text-blue-900 mb-2">Chemical Sciences</h4>
                    <p className="text-xs font-bold text-blue-700 leading-relaxed">Chemistry, Applied Chemistry, Biochemistry, and Forensic Science.</p>
                  </div>
                </div>
                <div className="space-y-4">
                  <div className="bg-purple-50 p-6 rounded-3xl border border-purple-100">
                    <h4 className="text-lg font-black text-purple-900 mb-2">Life Sciences</h4>
                    <p className="text-xs font-bold text-purple-700 leading-relaxed">Biology, Biotechnology, Microbiology, Botany, Zoology, and Genetics.</p>
                  </div>
                  <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100">
                    <h4 className="text-lg font-black text-slate-900 mb-2">Earth & Space</h4>
                    <p className="text-xs font-bold text-slate-700 leading-relaxed">Geology, Environmental Science, Marine Science, Atmospheric Science, and Geography.</p>
                  </div>
                </div>
              </div>
              <div className="mt-10 p-6 bg-slate-900 text-white rounded-3xl relative overflow-hidden">
                ...
                <h4 className="text-lg font-black mb-2 text-emerald-400">Interdisciplinary Fields</h4>
                <p className="text-sm font-medium leading-relaxed opacity-90">Modern science is increasingly interdisciplinary. Subjects like Data Science, Artificial Intelligence, and Bioinformatics are now core parts of prestigious B.Sc and M.Sc curriculums.</p>
              </div>
            </div>
          </div>
        )}
            ...
            >
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-end gap-6">
                    ...
                    <div className="flex-1 pb-2">
                      <div className="flex flex-wrap gap-2 mb-3">
                        <span className="bg-emerald-600 text-white text-[10px] font-black uppercase px-2.5 py-1 rounded-xl shadow-lg border border-emerald-400/30">NIRF Ranking #{selectedCollege.nirfRanking}</span>
                      ...
                      <div className="flex items-center gap-3 text-slate-300 text-xs sm:text-sm font-bold">
                        <div className="flex items-center gap-1.5"><MapPin className="w-4 h-4 text-emerald-400" /> {selectedCollege.city}, {selectedCollege.state}</div>
                        <div className="w-1.5 h-1.5 bg-slate-600 rounded-full"></div>
                        <div className="flex items-center gap-1.5"><Calendar className="w-4 h-4 text-emerald-400" /> Established {selectedCollege.yearEstablished}</div>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex-1 overflow-y-auto bg-slate-50">
                <div className="flex border-b bg-white px-8 sticky top-0 z-10 overflow-x-auto no-scrollbar shadow-sm">
                  {[
                    ...
                    { id: 'infrastructure', label: 'Facilities', icon: Building2 },
                    { id: 'research', label: 'Research', icon: Microscope },
                    { id: 'placements', label: 'Career', icon: Briefcase },
                    ...
                    <button
                      ...
                      className={`flex items-center gap-2.5 px-8 py-5 text-xs font-black transition-all border-b-[3px] shrink-0 cursor-pointer ${
                        modalTab === tab.id ? 'border-emerald-600 text-emerald-600' : 'border-transparent text-slate-400 hover:text-slate-600'
                      }`}
                    >
                  ...
                  {modalTab === 'academics' && (
                    <div className="space-y-10 animate-in fade-in duration-300">
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm">
                          <h4 className="text-base font-black text-slate-900 mb-6 flex items-center gap-3">
                            <Layers className="w-5 h-5 text-emerald-600" /> Academic Degrees
                          </h4>
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {selectedCollege.programmes.map((prog, idx) => (
                              <div key={idx} className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100 group hover:border-emerald-300 transition">
                                <Award className="w-5 h-5 text-emerald-400 group-hover:scale-110 transition" />
                        ...
                        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm">
                          <h4 className="text-base font-black text-slate-900 mb-6 flex items-center gap-3">
                            <Atom className="w-5 h-5 text-emerald-600" /> Major Subjects
                          </h4>
                          <div className="flex flex-wrap gap-2.5">
                            {selectedCollege.subjects.map((sub, idx) => (
                              <span key={idx} className="bg-emerald-50 text-emerald-700 text-xs font-black px-4 py-2 rounded-xl border border-emerald-100 flex items-center gap-2">
                      ...
                      <div className="bg-slate-900 text-white rounded-[2.5rem] p-10 relative overflow-hidden">
                        <div className="absolute right-0 top-0 p-12 opacity-10"><FlaskConical className="w-40 h-40" /></div>
                        <h4 className="text-xl font-black mb-4 text-emerald-400 italic">Academic Excellence</h4>
                        <p className="text-sm font-medium text-slate-300 leading-relaxed mb-8 max-w-3xl">
                          Affiliated with {selectedCollege.affiliatedUniversity}, this institution maintains high academic standards with regular curriculum updates to include modern scientific advancements.
                        </p>
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                          <div className="bg-white/10 p-5 rounded-3xl border border-white/10 backdrop-blur-md">
                            ...
                            <p className="text-[10px] font-black text-emerald-200 uppercase mb-1">Principal / Director</p>
                            <p className="text-sm font-black">{selectedCollege.principal}</p>
                          </div>
                          <div className="bg-white/10 p-5 rounded-3xl border border-white/10 backdrop-blur-md">
                            <Building className="w-6 h-6 text-emerald-400 mb-2" />
                            <p className="text-[10px] font-black text-emerald-200 uppercase mb-1">Ownership</p>
                            <p className="text-sm font-black">{selectedCollege.ownership}</p>
                          </div>
                          <div className="bg-white/10 p-5 rounded-3xl border border-white/10 backdrop-blur-md">
                            <Calendar className="w-6 h-6 text-emerald-400 mb-2" />
                            <p className="text-[10px] font-black text-emerald-200 uppercase mb-1">Established</p>
                            <p className="text-sm font-black">{selectedCollege.yearEstablished}</p>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-8 animate-in fade-in duration-300">
                      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        <div className="lg:col-span-2 bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm">
                          <h4 className="text-base font-black text-slate-900 mb-6 flex items-center gap-3">
                            <FileText className="w-5 h-5 text-emerald-600" /> Eligibility & Entrance
                          </h4>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                            <div>
                              <p className="text-[10px] font-black text-slate-400 uppercase mb-4 tracking-widest ml-1">Accepted Exams</p>
                        ...
                        <div className="flex flex-col gap-4">
                          <a href={selectedCollege.admissionPortalUrl} target="_blank" rel="noopener noreferrer" className="bg-emerald-600 text-white p-6 rounded-[2rem] text-center shadow-xl hover:bg-emerald-700 transition group flex flex-col items-center justify-center">
                            <FastIcon className="w-8 h-8 mb-3 text-white/50" />
                            <span className="text-sm font-black">Official Admission Portal</span>
                            <span className="text-[10px] opacity-60 mt-1">Apply for 2024 Academic Year</span>
                          </a>
                          <a href={selectedCollege.counsellingPortalUrl} target="_blank" rel="noopener noreferrer" className="bg-white text-slate-900 p-6 rounded-[2rem] text-center border-2 border-slate-900 hover:bg-slate-50 transition flex flex-col items-center justify-center">
                            ...
                            <span className="text-sm font-black">Counselling Portal</span>
                            <span className="text-[10px] text-slate-400 mt-1">Check Allotment & Cutoffs</span>
                          </a>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-8 animate-in fade-in duration-300">
                      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-5">
                        {selectedCollege.infrastructure.map((item, idx) => (
                          <div key={idx} className="bg-white p-5 rounded-[2rem] border border-slate-200 text-center hover:border-emerald-400 transition shadow-sm group">
                            <div className="w-12 h-12 bg-slate-50 rounded-2xl flex items-center justify-center mx-auto mb-3 text-slate-300 group-hover:bg-emerald-50 group-hover:text-emerald-500 transition shadow-inner">
                  ...
                  {modalTab === 'research' && (
                    <div className="animate-in fade-in duration-300">
                      <div className="bg-white p-10 rounded-[3rem] border border-slate-200 shadow-sm relative overflow-hidden">
                        <div className="absolute right-0 top-0 p-12 opacity-5"><FlaskConical className="w-48 h-48" /></div>
                        <h4 className="text-base font-black text-slate-900 mb-6 flex items-center gap-3">
                          <Microscope className="w-6 h-6 text-emerald-600" /> Research & Innovation
                        </h4>
                        <p className="text-sm font-bold text-slate-600 leading-relaxed max-w-4xl mb-8">
                          {selectedCollege.research}
                        </p>
                        <div className="flex flex-wrap gap-4">
                          <div className="bg-emerald-50 px-6 py-4 rounded-2xl border border-emerald-100">
                            <p className="text-[10px] font-black text-emerald-400 uppercase mb-1">Collaboration</p>
                            <p className="text-xs font-black text-emerald-900">National & International Ties</p>
                          </div>
                          <div className="bg-blue-50 px-6 py-4 rounded-2xl border border-blue-100">
                            <p className="text-[10px] font-black text-blue-400 uppercase mb-1">Labs</p>
                            <p className="text-xs font-black text-blue-900">Advanced Research Facilities</p>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-10 animate-in fade-in duration-300">
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm text-center relative overflow-hidden group">
                          ...
                          <p className="text-[10px] font-bold text-emerald-600">Scientific & Corporate Roles</p>
                        </div>
                        <div className="bg-slate-900 p-8 rounded-[2.5rem] border border-slate-800 shadow-xl text-center relative overflow-hidden group">
                          <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:scale-110 transition duration-500"><Award className="w-20 h-20 text-emerald-400" /></div>
                          <p className="text-[10px] font-black text-emerald-300 uppercase mb-2 tracking-widest">Highest Package</p>
                          <p className="text-3xl font-black text-emerald-400 mb-1">{selectedCollege.highestPackage}</p>
                          <p className="text-[10px] font-bold text-white/40">Verified Placement Record</p>
                        </div>
                      ...
                      <div className="bg-white p-10 rounded-[3rem] border border-slate-200 shadow-sm">
                        <h4 className="text-sm font-black text-slate-400 uppercase text-center mb-8 tracking-[0.2em]">Top Recruiting Partners</h4>
                    ...
                    <div className="space-y-8 animate-in fade-in duration-300">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="bg-white p-10 rounded-[3rem] border border-slate-200 shadow-sm relative overflow-hidden">
                          ...
                          <h4 className="text-base font-black text-slate-900 mb-8">Estimated Fee Structure</h4>
                          <div className="space-y-6">
                            ...
                            <div className="flex justify-between items-end border-b pb-6 border-slate-100">
                              <div>
                                <p className="text-[10px] font-black text-slate-400 uppercase mb-1">Hostel & Mess</p>
                                <p className="text-sm font-bold text-slate-500">Accommodation Charges</p>
                              </div>
                              <p className="text-2xl font-black text-slate-900">{selectedCollege.hostelFees}</p>
                            </div>
                          </div>
                        </div>
                        <div className="bg-emerald-600 p-10 rounded-[3rem] text-white shadow-xl flex flex-col justify-center relative overflow-hidden">
                          <div className="absolute top-0 right-0 p-8 opacity-10 rotate-12"><Sparkles className="w-32 h-32" /></div>
                          <h4 className="text-xl font-black mb-6">Financial Support</h4>
                          ...
                          <p className="text-[11px] font-medium opacity-80 leading-relaxed italic">
                            * Students are encouraged to apply for the INSPIRE Scholarship (Innovation in Science Pursuit for Inspired Research) if they are in the top 1% of their board results.
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-8 animate-in fade-in duration-300">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm space-y-6">
                          <h4 className="text-base font-black text-slate-900 mb-2">Campus Contact Details</h4>
                          ...
                          <div className="flex items-start gap-4">
                            ...
                            <div>
                              <p className="text-[10px] font-black text-slate-400 uppercase mb-1">General Enquiry</p>
                        ...
                        <div className="h-[400px] bg-slate-200 rounded-[2.5rem] overflow-hidden border border-slate-300 shadow-inner relative group">
                          ...
                          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                            <a href={selectedCollege.googleMapsUrl} target="_blank" rel="noopener noreferrer" className="bg-slate-900 text-white px-6 py-3 rounded-2xl text-xs font-black shadow-2xl pointer-events-auto hover:bg-emerald-600 transition flex items-center gap-2">
              ...
              <div className="bg-white border-t p-8 flex flex-wrap gap-4 items-center justify-between shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
                <div className="flex items-center gap-4 text-xs text-slate-400 font-bold">
                  <div className="flex items-center gap-1.5"><ShieldCheck className="w-4 h-4 text-emerald-500" /> UGC Recognised</div>
                ...
                <div className="flex gap-3">
                  ...
                  <a href={selectedCollege.website} target="_blank" rel="noopener noreferrer" className="bg-emerald-600 text-white px-10 py-4 rounded-2xl text-sm font-black hover:bg-emerald-700 transition shadow-xl shadow-emerald-500/20 flex items-center gap-2">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 mt-12 flex justify-center">
        <div className="bg-white border border-slate-200 rounded-full px-8 py-4 flex items-center gap-6 shadow-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-black text-slate-500 uppercase tracking-widest">Verified Scientific Sources</span>
          </div>
          <div className="w-px h-4 bg-slate-200"></div>
          <div className="flex items-center gap-2">
            <Award className="w-5 h-5 text-emerald-500" />
            <span className="text-xs font-black text-slate-500 uppercase tracking-widest">NIRF & NAAC Integrated</span>
          </div>
        </div>
      </div>
